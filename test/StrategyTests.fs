...
let ``AlwaysHits always hits and AlwaysStands always stands`` () =
    Assert.Equal(Strategy.Hit, Strategy.Decide AlwaysHits 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide AlwaysStands 0u 0u player [] [] decks)
...
let ``RandomWithProbability is deterministic at the extremes`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (RandomWithProbability 1.0) 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (RandomWithProbability 0.0) 0u 0u player [] [] decks)
...
let ``HitUntilScore hits below the threshold and stands at it`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilScore 16u) 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilScore 15u) 0u 0u player [] [] decks)

[<Fact>]
let ``HitUntilNumCards hits below the threshold and stands at it`` () =
    // The player's hand has 2 cards
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilNumCards 3u) 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilNumCards 2u) 0u 0u player [] [] decks)
...
let ``HitUntilBustProbability hits below the threshold and stands above it`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilBustProbability 0.2) 0u 0u player [ other ] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilBustProbability 0.1) 0u 0u player [ other ] [] decks)
...
let ``HitUntilBustProbability counts deal3 but not freeze as a bust when alone`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilBustProbability 0.17) 0u 0u player [ other ] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilBustProbability 0.17) 0u 0u player [] [] decks)
...
let ``HitUntilNaiveBustProbability ignores the actual deck composition`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilNaiveBustProbability 0.2) 0u 0u player [ other ] [] poisoned)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilBustProbability 0.2) 0u 0u player [ other ] [] poisoned)
...
let ``SoftHitUntilScore is deterministic far from the threshold`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (SoftHitUntilScore(20u, 0.1)) 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (SoftHitUntilScore(10u, 0.1)) 0u 0u player [] [] decks)
...
let ``HitUntilTotal counts banked points as well as the hand`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilTotal 116u) 0u 0u banked [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilTotal 115u) 0u 0u banked [] [] decks)
...
let ``HitUntilUniqueValues ignores modifier cards`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitUntilUniqueValues 3u) 0u 0u modified [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitUntilUniqueValues 2u) 0u 0u modified [] [] decks)
...
let ``ChasesFlip7 keeps hitting near the bonus regardless of score`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (ChasesFlip7(20u, 6u)) 0u 0u sixUniques [] [] decks)
    // With only two uniques the score threshold applies as usual
    Assert.Equal(Strategy.Stand, Strategy.Decide (ChasesFlip7(15u, 6u)) 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Hit, Strategy.Decide (ChasesFlip7(16u, 6u)) 0u 0u player [] [] decks)
...
let ``EmboldenedBySecondChance hits fearlessly while holding one`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (EmboldenedBySecondChance 15u) 0u 0u insured [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (EmboldenedBySecondChance 15u) 0u 0u player [] [] decks)
...
let ``HitWhileBehindLeader races the visible table totals`` () =
    ...
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitWhileBehindLeader 0u) 0u 0u player [ rival ] [] decks)

    // Ahead 115 to 31 stands, unless the margin demands a bigger lead
    let banked = { player with FirmScore = 100u }
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitWhileBehindLeader 0u) 0u 0u banked [ rival ] [] decks)
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitWhileBehindLeader 100u) 0u 0u banked [ rival ] [] decks)

    // A leader who already stood still counts: their locked hand shows 116
    let stood = { rival with FirmScore = 115u }
    Assert.Equal(Strategy.Hit, Strategy.Decide (HitWhileBehindLeader 0u) 0u 0u banked [] [ stood ] decks)
    ...
    Assert.Equal(Strategy.Stand, Strategy.Decide (HitWhileBehindLeader 0u) 0u 0u banked [] [ busted ] decks)

[<Fact>]
let ``StandsAfterTurn hits up to and including the given turn`` () =
    // Turn 3 is at the threshold; turn 4 is past it
    Assert.Equal(Strategy.Hit, Strategy.Decide (StandsAfterTurn 3u) 0u 3u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide (StandsAfterTurn 3u) 0u 4u player [] [] decks)

[<Fact>]
let ``MaximizesExpectedValue hits on a fresh deck and stands when every card busts`` () =
    Assert.Equal(Strategy.Hit, Strategy.Decide MaximizesExpectedValue 0u 0u player [] [] decks)
    Assert.Equal(Strategy.Stand, Strategy.Decide MaximizesExpectedValue 0u 0u player [] [] (allSevens, Deck.Empty))
