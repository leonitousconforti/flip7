...
module public Strategy =
    ...
    /// the decks. Deciders are asynchronous so a decision can be awaited (a
    /// key press, a network message) without holding a thread. DecideWith is
    /// the canonical Decider; injecting a different one into
    /// Timeline.SimulateWithDecider lets Prompt strategies be decided by a
